...
This project focuses on the analysis of the data job market, focusing primarily on the Data Analyst roles in Data field. The main aim of this project is to help understand and navigate the job market more effectively. It points out the top-paying and in-demand skills that appear in job postings to help job seekers find optimal job opportunities. 

The data used for this entire project is provided by Luke Barousse and is from 2023 so the data might differ in comparison to the data of the job postings from the 2025, but still should be able to help and understand certain trends on the Data job market. 
...
Ten projekt wykonany w Pythonie został stworzony na podstawie kursu do Pythona autorstwa Luke Barousse'a. Link do kursu: https://www.youtube.com/watch?v=wUSDVGivd-8.

Dzięki temu poradnikowi zrozumiałem podstawowe aspekty związane z Pythonem takie jak:
- zmienne
- rodzaje danych
- operatory
- funkcje
...
- standardowe biblioteki Pythona
    ...
    - random
    - ast
- biblioteki związane z Data Science
    - Numpy
    - Matplotlib
    - Seaborn
    - Pandas

Projekt ten skupia się na analizie danych dotyczących rynku pracy, skupiając się główie na roli Analityka Danych. Głównym celem tego projekt jest pomoc w zrozumieniu i w prostszym i efektywniejszym "poruszaniu" się po rynku pracy. Wypunktowane są najlepiej opłacane i najczęściej wymagane umiejętności, które pojawiają się w ofertach pracy, aby pomóc osobom poszukującym pracy znaleźć jak najlepszą ofertę.

Dane wykorzystywane w tym projekcie zostały dostarczone przez Luke Barousse'a. Pochodzą one z roku 2023, a więc mogą się różnić od danych dotyczących ofert pracy z aktualnego roku czyli 2025 r., ale nadal powinny pomóc w zwizualizowaniu i zrozumieniu niektórych trendów na rynku pracy w branży Data.

Wykorzystując odpowiednie skrypty i biblioteki dostępne w Pythonie, projekt ten analizuje najważniejsze kwestie związan z ofertami pracy:

- najczęściej poszukiwane umiejętności na rynku pracy
- wynagrodzenie
- zależnośc między cieszącymi się popytem umiejętnościami a oferowanym wynagrodzeniem w dziedzinie analizy danych.

## **Pytania, na które odpowiada ten projekt**
1. Jakie umięjętności są najbardziej pożądane w trzech najlpopularniejszych zawodach zajmującymi się danymi?
2. Jakie są trendy w zakresie pożądanych umiejętności u analityków danych?
3. Jakie są zarobki w zawodach związanych z danymi i jakie są najczęsciej pożądane umiejętności u analityków danych?
4. Które umiejętności są najoptymalniejsze dla analityków danych? (Pod względem pożądanych umiejętności i płacy z nimi związanej)

## **Narzędzia wykorzystane w tym projekcie**

Aby stworzyć ten projekt, nauczyłem się korzystać z następujących narzędzi:

**Python**: główne narzędzie całej analizy.
**Biblioteka Pandas**: biblioteka Pythona ułatwiająca analizę danych.
**Biblioteka Matplotlib**: biblioteka Pythona umożliwiająca wizualizację danych. 
**Biblioteka Seaborn**: bibliotek Pythona służąca do dalszego dostosowywania i ulepszania wizualizacja tworzonych za pomocą biblioteki Matplotlib.
**Jupyter Notebooks**: narzędzie umożliwiające egzekwowanie skryptów Pythona, analizę i tworzenie notatek podczas całego procesu.
**Visual Studio Code**: potężny edytor kodu umożliwiający egzekwowanie skryptów Python
**Git & GitHub**: umożliwia version control, a także udostępianie kodu Python stojącego za tym projektem. 

## **Przygotowanie Danych**

Ta sekca wyjaśnia w jaki sposób dane zostały przygotowane przed analizą.

**Importowanie i czyszczenie danych**

Za każdym razem zaczynałem poprzez załadowanie zestawu danych jak i importowanie bibliotek Pytona niezbędnych do analizy danych
...
Aby zawężyć zakres analizy, skupiłem się na ofertach pracy opublikowanych w Stana Zjednoczonych ze względu na wysoką ilość tych ofert. 

    df_US = df[df['job_country'] == 'United States]

**Filtrowanie zawodu analityka danych**

    df_US = df[df['job_title_short'] == 'Data Analyst']

## **Analiza**

W każdym Jupyter Notebook w tym kursie znajduję się moja "próba" zrozumienia i nauczenia się obsługi Pythona w celu analizy poszczególnych aspektów rynku pracy.

**1. Jakie umiejętności są najbardziej pożadane w Top 3 najpopularniejszych tytutał w Data Science?**
---

Aby znaleźć najbardziej porządane umiejętności dla pozycji zajmujących się danymi, najpierw znalazłem trzy najpopularniejsze tytuły z ośmiu ogólnych tytułów zawartych w kolumnie 'job_title_short' poprzez zsumowanie ogólnej ilości ofert pracy dla każdego z ośmiu tytułów. Następnie, znalazłem top 5 najczęściej wspominanych umięjętności w tych ofertach pracy dla każdego tytułu.

Query może być znalezione [tutaj](/3_Python_Project/2_Skills_Count.ipynb)
...
![Prawdopodobieństwo wystąpienia danych umiejętności w ofertach pracy w USA](/3_Python_Project/images/likelihood_of_skills.png)

*Prawdopodobieństwo wystąpienia danych umiejętności w ofertach pracy w USA*

**Spostrzeżenia**
- SQL jest najbardziej pożądaną umiejętnością w wypadku Data Analysts (51%) jak i Data Engineers (68%). Jest on obecny w ponad połowie wystawionych ofert pracy dla tych dwóch ról. W przypadku Data Scientists, najbardziej pożądaną umiejętnością był Python (osiągający wartość 72%).
- Python jest wszechstronnym narzędziem będącym obecnym w sporej części ofert pracy dla trzech najpopularniejszych tytułów zajmujących się danymi. (Data Analyst - 27%, Data Engineer - 65%, Data Scientist - 72%).

**2. Jakie panują trendy dotyczące pożądanych umiejętności u analityków danych?**
---

W celu znalezienia pożądanych umiejętności w wypadku analityków danych, przefiltrowałem wszystkie oferty pracy pod względem dwóch rzeczy, kraju i tytułu (USA i Analityk Danych), a następnie przez miesiąc aby stworzyć wykres pokazujący popularność danych umiejętności w 2023 roku. 

Query można znaleźć [tutaj](/3_Python_Project/3_Skills_Trend.ipynb)
...
Query można znaleźć [tutaj](/3_Python_Project/4_Salary_Analysis.ipynb)
...
**Spostrzeżenia**
- Mediana wynagrodzeń wzrasta razem z osiągnięciem tytułu Seniora odpowiadających ról. Jednakże, w wypadku analityków danych i starszego analityka danych, pozostaje ona w niższych przedziałach w porównaniu z innymi rolami. 
- Największą mediane wynagrodzeń osiąga Data Scientist z wartościami odstającymi osiągającymi ponad 500 tysięcy dolarów (jednak, wartości te są dość rzadkie i mogą być traktowane jako wyjątki od reguły zwłaszcza, że takie wyjątki nie występują w wypadku Senior Data Scientist). Te wartości odstające od normy sugerują, że pracodawcy wysoko cenią sobie doświadczenie i zaawansowane umiejętności związane z danymi.

**Najlepiej opłacane i najbardziej pożądane umiejętności u analityków danych**
...
![Najlepiej opłacane, najbardziej pożadane a za razem najpopularniejsze umiejętności analityków danych](/3_Python_Project/images/top_paid_&_popular_skills.png)

*Dwa oddzielne wykresy pokazujące najlepiej płatnie i najbardziej pożądane umiejętności analityków danych w USA*

**4. Jaka umiejętność jest najbardziej opłacalna do nauczenia się przez analityków danych?**

Query może zostać znalezione [tutaj](/3_Python_Project/5_Optimal_Skills.ipynb)
...
![Najbardziej opłacalna umiejętność dla analityków danych w USA](/3_Python_Project/images/most_optimal_skills_DA.png)

*Wykres punktowy przedstawiający najbardziej optymalne umiejętności dla analityków danych (pod względem wysokości wynagrodzenia i zapotrzebowania).

**Spostrzeżenia**
- Wykres punktowy pokazuje, że większość umiejętności programowania (oznaczonych kolorem niebieskim) mają tendencje pozyskiwania wyższych płac względem innych umiejętności wymaganych od analityków danych co sugeruje, że znajomość języków programowania może pozytywnie wpłynąć na wynagrodzenie.
- Narzędzia analityczne (oznaczone kolorem zielonym), do których zalicza się PowerBI czy też Tableau, często pojawiają się w ofertach pracy pokazując, że wizualizacja danych jest ważnym aspektem pracy analityka danych. Umiejętności te nie tylko zapewniają wystarczającą wypłate ale także różnorakie możliwości. 
- Umiejętności związane z bazami danych takimi jak Oracle czy też SQL Server zapewniają jedne z wyższych wynagrodzeń wśród analityków danych. Wskazuje to na znaczące zapotrzebowanie na osoby władające umiejętności zarządzania i manipulowania danymi. 

# Podsumowanie

Cały ten projekt był dla mnie doskonałą okazją do nauki nowych umiejętności jak i doskonalenia już opanowanych umiejętności, które będą mi potrzebne jako przyszły analityk danych.
Projekt ten ujawnia informacje dotyczące rynku pracy w branży danych, podkreślając niezbędne umiejętności jak i ważne trendy, które pozwalają ludziom stać się częścią tej interesującej dziedziny. 
Ponieważ dane dostarczone do tego projektu zawieraja informacje dotyczące tylko 2023 roku, może być koniecznie odtworzeie tego projektu na podstawie nowszych danych dotyczących bieżącego roku.
Jednak projek ten nadal stanowi doskonałe narzędzie umożliwiające analize i przystępniejszze zrozumienie rynku pracy w zakresie ról związanych z danymi. Dzięki temu przygotowania do wejścia w tę dziedzinę będą znacznie łatwiejsze, ponieważ, teraz wiadomo na jakich umiejętnościach opłaca się skupić.
Jeszcze raz dziękuje Luke'owi Barousse'owi za jego wspaniałe kursy, które są dostępne na YouTube!
